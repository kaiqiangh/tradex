...
Version-1 adds primary Trade UI command `binance.testnet.orders.cancel`, accepting `{workspaceId, connectionId, expectedConnectionStateVersion, symbol, providerOrderId, expectedBookStateVersion, idempotencyKey, confirmed}` and returning the saved `BinanceTestnetOrderBook`. Only connected `binance` / `TESTNET` accounts and exact `BTCUSDT` / `ETHUSDT` orders in `NEW` or `PARTIALLY_FILLED` state with a known, valid remaining quantity are eligible. The saved order observation must be `CURRENT`, match the account/book versions and remote identity, and be no more than 60 seconds old. A transaction stores `SUBMITTING`, its UUID idempotency key, a new book version and outbox event before provider I/O. A reopened `SUBMITTING` attempt becomes `PENDING` with unknown outcome and is never resent.
