...
- [S15 回测运行生命周期证据](s15-backtest-evidence.md)：#48/#49/#50 的 schema、SQLite、状态生命周期、deterministic result、manifest、guard、双入口 compare、浏览器 fixture 与验证命令。
- [S17 Alpaca Paper 交易生命周期 Spec](s17-alpaca-paper.md)：Alpaca Paper 账户观测、订单/成交、未知提交恢复、撤单和私有流的实现规范；#57 提交/未知恢复、#58 订单/成交/撤单（依赖 #57）、#59 私有流/重连（依赖 #58）。
...
10. 执行方案覆盖规范正文中的行为，不止 203 个编号：Watchlists、新闻/filings/fundamentals、归档校验、许可证元数据、审计保留、沙箱、键盘焦点、窗口布局等均在下方分配。新 IPC 操作先补双语契约，再生成/校验 Rust 与 TS 类型。
