...
- [S18 Trading 212 Demo 交易生命周期 Spec](s18-trading212-demo.md)：#60 已完成官方 API / 现有实现边界梳理。#61 提交与未知结果恢复已完成，代码提交 `873d42f`、验收证据见 [#61 evidence](s18-trading212-demo-submit-evidence.md)；实现票为 [#62 订单簿与累计成交](https://github.com/kaiqiangh/tradex/issues/62)、[#63 撤单与成交竞态](https://github.com/kaiqiangh/tradex/issues/63)；[#64 真实 Demo 验收 gate](https://github.com/kaiqiangh/tradex/issues/64) 依赖前三项并需人工确认。按用户要求串行执行 #62 → #63 → #64；#62 可独立查询既有账户订单，但不会并行推进。S17 #56 按用户指示保持 OPEN。
