...
- [S15 回测运行生命周期证据](s15-backtest-evidence.md)：#48 的 schema、SQLite、状态生命周期、浏览器 fixture 与验证命令。
