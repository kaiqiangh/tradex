...
- Browser fixture 已检查 Login/Configure/Verify 的门控、错误保留、窄屏和秘密不进入 renderer；不使用用户 broker/API/OAuth 凭据。Tauri secure dialog 的 Cmd-Return/Save 以及 OAuth 取消、过期、超时 UI 仍需独立运行证据，不能用 fixture 标记 Ready。
