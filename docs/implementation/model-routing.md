...
状态：**VERIFIED（#13 验收完成）**。默认路由、显式 fallback 同意、provider-specific cooldown、attempt 审计和 Rust-backed UI 已通过；本票仍不实现 S04 的 Codex Thread/Turn 流式生命周期，后续由 S04 消费本票的 planner 与 attempt contract。
