...
| `OD-003` | SEC EDGAR `data.sec.gov` submissions 与 XBRL Company Facts/Frames | filings/XBRL 的更新时间由 SEC 发布；字段适用范围以 SEC schema/form 为准。 | 当前公开 JSON API 不要求 API key；每次自动请求必须带可识别 User-Agent 并遵守 fair-access（≤10 req/s）和隐私/使用政策。公开可读不等于 TradeX 获得再分发或商业许可，导出须保留来源与法律状态。 | 未能读取或字段不完整时，基本面字段标记 unavailable/stale，相关 Research 卡片禁用或降级为带原因的 `UNAVAILABLE`。 |
...
- 2026-09-13 只读探测摘要：SEC `data.sec.gov/submissions/CIK0000320193.json` 返回 HTTP 200；ECB `EXR/D.USD.EUR.SP00.A` CSV 返回 HTTP 200；Alpaca calendar 在无 API headers 时返回 HTTP 401。响应正文未写入仓库。详见 S06 evidence。
