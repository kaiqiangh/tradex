...
日期：2026-09-14  
开发分支：`dev`  
实现提交：`12218be` (`feat: add read-only portfolio valuation view`)

## Delivered slice

- Added the versioned `portfolio.get` Control Plane command and generated Rust/JSON Schema/TypeScript contract.
- Aggregated stored account balances, positions and open orders without writing SQLite, outbox, account, model, risk or thread state. Provider fills remain an explicit unavailable value when an adapter does not expose them.
...
cargo test --workspace            # 69 unit/lib tests plus integration suites passed; 6 native Keychain/gateway checks ignored by contract
cargo clippy --workspace --all-targets --all-features -- -D warnings
git diff --check
python3 scripts/check_requirements.py
```

The focused portfolio checks include exact signed decimal addition/multiplication, deterministic fixture routes and USDT depeg warning, empty production behavior, malformed payload rejection, workspace scoping and a before/after domain snapshot equality assertion.
