...
- 撤单前必须针对同一 Demo connection/provider order ID 刷新当前 pending order，检查其 provider identity、status、filled / remaining quantity 和 observed time，再由用户查看这些细节并明确确认。后端持久化一次 cancel attempt，然后最多发送一个 DELETE。成功 DELETE 只表示撤单已接受，映射为 `CANCEL_PENDING`；DELETE 超时/未知响应同样进入 pending/unknown 且不重发。之后查询 open/history 对账，终态撤单或成交竞态以 provider 事实收敛。不得将撤单改造成隐式 replace/new order。
