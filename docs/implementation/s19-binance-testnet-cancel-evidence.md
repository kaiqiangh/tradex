...
- `cargo test --test binance -- --test-threads=1` — 16 passed, 0 failed, 1 ignored (native Keychain).
...
- Rust-backed browser fixture — exact Detail refresh occurs before the dialog; stale observations block review; Escape sends no command; explicit confirmation reaches `CANCELED` while preserving an earlier partial fill; responsive checks passed at 390, 768, and 1280 px.
