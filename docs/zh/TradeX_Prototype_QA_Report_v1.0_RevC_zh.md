...
当前状态为 10 个 FAILED、3 个 PARTIAL 场景。它们是回归场景计数，不是 FR/AC 覆盖率。规范可用于指导修复；原型交付须通过下面的关键场景以及独立 QA 门槛。
...
### QA-13 — Trading 212 Demo 本地账户永久删除
...
1. 修复后重跑 QA-01–QA-13，保存源码版本、初始状态、操作、预期/实际和截图/状态断言证据。
