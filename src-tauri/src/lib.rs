...
use protocol::{
    ...
    BinanceTestnetHistoryState, BinanceTestnetOrderAttemptQuery,
    ...
    Trading212DemoOrderBookStatus, Trading212DemoOrderCancel, Trading212DemoOrderSubmit,
...
fn empty_binance_testnet_order_book(
    account: &AccountConnection,
    ...
    Ok(BinanceTestnetOrderBook {
        ...
        rate_limits: BinanceTestnetOrderBookRateLimits {
            pending_orders_retry_at: None,
            account_retry_at: None,
            history_retry_at: None,
            order_detail_retry_at: None,
        },
        history: ["BTCUSDT", "ETHUSDT"]
            .into_iter()
            .map(|symbol| BinanceTestnetHistoryState {
                symbol: symbol.into(),
                started: false,
                complete: false,
                page_count: 0,
...
impl ControlPlane {
    ...
    fn execute(
        ...
        match request.command.as_str() {
            ...
            "binance.testnet.orders.get" => {
                let input: BinanceTestnetOrderBookQuery = payload(request.payload)?;
                self.require_workspace(&input.workspace_id)?;
                let store = self.store.as_ref().unwrap();
                let book = match store
                    .binance_testnet_order_book(&input.workspace_id, &input.connection_id)?
                {
                    Some(book) => Some(book),
                    None => Some(empty_binance_testnet_order_book(
                        &store.account(&input.connection_id)?,
                    )?),
                };
                Ok((json!(BinanceTestnetOrderBookQueryResult { book }), None))
            }
            "alpaca.paper.order.submit" => {
                if !provider_order_consumer_allowed(consumer) {
                    return Err(TradeXError::new("ORDER_SUBMIT_FORBIDDEN"));
                }
                let input: AlpacaPaperOrderSubmit = payload(request.payload)?;
                self.require_workspace(&input.workspace_id)?;
                let attempt = self
                    .store
                    .as_ref()
            ...
            "alpaca.paper.order.reconcile" => {
                ...
                let input: AlpacaPaperOrderReconcile = payload(request.payload)?;
                self.require_workspace(&input.workspace_id)?;
                let attempt = self
                    .store
                    .as_ref()
    ...
    pub fn prepare_provider(&mut self, value: &Value) -> Result<Option<ProviderJob>> {
        self.prepare_provider_for(value, "headless")
    }

    pub fn prepare_provider_for(
        &mut self,
        value: &Value,
        consumer: &str,
    ...
    fn prepare_alpaca_paper_order_submit(
        &mut self,
        request: CommandEnvelope,
        consumer: &str,
    ) -> Result<Option<ProviderJob>> {
            ...
            .alpaca_paper_order_attempt(&input.workspace_id, &input.proposal_id)?
        {
            ...
            let (_, created) = self
                ...
                .as_mut()
                .unwrap()
                .begin_alpaca_paper_order_attempt(&input)?;
            if !created {
                return Ok(None);
            }
        }
    ...
    fn prepare_alpaca_paper_order_reconcile(
        &mut self,
        request: CommandEnvelope,
        consumer: &str,
    ) -> Result<Option<ProviderJob>> {
        ...
        if attempt.state != AlpacaPaperOrderAttemptState::UnknownReconciling {
            return Ok(None);
        }
        let proposal = self
            .store
    ...
    fn prepare_binance_testnet_order_book_refresh(
        &mut self,
        request: CommandEnvelope,
        consumer: &str,
    ) -> Result<Option<ProviderJob>> {
        ...
        let input: BinanceTestnetOrderBookRefresh = payload(request.payload)?;
        self.require_workspace(&input.workspace_id)?;
        let account = self.current_account(
            &input.workspace_id,
            &input.connection_id,
        ...
        let valid = match input.action {
            BinanceTestnetOrderBookAction::Pending | BinanceTestnetOrderBookAction::Account => {
                input.symbol.is_none() && input.provider_order_id.is_none()
            }
        ...
        Ok(Some(ProviderJob {
            account,
            kind: JobKind::BinanceTestnetOrderBookRefresh,
            session: self.session.clone(),
            request_id: request.request_id,
            trading212_demo_attempt: None,
            trading212_demo_proposal: None,
    ...
    pub fn complete_provider(&mut self, job: &ProviderJob, outcome: ProviderOutcome) -> Value {
        ...
        if job.kind == JobKind::BinanceTestnetOrderBookRefresh {
        ...
        if matches!(
            ...
            let attempt = outcome.alpaca_paper_attempt.or_else(|| {
                job.alpaca_attempt.clone().map(|mut attempt| {
                    attempt.state = AlpacaPaperOrderAttemptState::UnknownReconciling;
                    attempt.error_code = Some("ORDER_STATUS_UNKNOWN".into());
                    attempt.reason = "Provider outcome was unavailable. Query by client order ID before taking any further action.".into();
                    attempt
                })
            });
            let Some(attempt) = attempt else {
            ...
            return match self
                ...
                .as_mut()
                .unwrap()
                .complete_alpaca_paper_order_attempt(&attempt)
            {
                Ok(attempt) => json!({
                    "requestId":job.request_id,
                    "schemaVersion":1,
...
mod thread_tests {
    ...
    fn artifact_round_trip_preserves_provenance_and_exports_safely() {
        ...
        assert_eq!(migrated_open["ok"], true, "{migrated_open}");
        assert_eq!(
