...
function LocalPaperSummary({ state }: { state: LocalPaperState }) {
  ...
  return <section className="card local-paper-summary" aria-labelledby="local-paper-summary-title">
    ...
    {(state.orders ?? []).length > 0 && <section aria-label="Local Paper order history"><h3>Local Paper order history</h3><ul>{(state.orders ?? []).map(order => { const fills = state.fills.filter(fill => fill.orderId === order.orderId); const events = (state.events ?? []).filter(event => event.orderId === order.orderId); return <li key={order.orderId}><strong>{order.state}</strong> · {order.instrumentId} · {order.filledQuantity} filled · {order.remainingQuantity} remaining{['ACCEPTED', 'PARTIALLY_FILLED'].includes(order.state) && <button type="button" onClick={() => requestCancel(order.orderId)} disabled={busyOrder === order.orderId}>{busyOrder === order.orderId ? 'Cancelling…' : 'Cancel'}</button>}<details><summary>View fills and events</summary><p>Fills</p>{fills.length ? <ul>{fills.map(fill => <li key={fill.fillId}>{fill.quantity} @ {fill.price} {fill.currency} · {time(fill.observedAt)}</li>)}</ul> : <p>No fills.</p>}<p>Events</p>{events.length ? <ol>{events.map(event => <li key={event.eventId}>{event.kind} · {time(event.occurredAt)} · sequence {event.sequence}</li>)}</ol> : <p>No events.</p>}</details></li>; })}</ul></section>}
