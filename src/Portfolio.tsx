...
function observationText(observedAt: string, health: AccountHealth) {
  return <><span>{observedAt}</span><small className="identity">{health.connection} · {health.reconciliation}</small></>;
}

function orderAmount(order: PortfolioOrder) {
  if (order.quantity != null) return order.quantity;
  if (order.notional != null) return `${order.notional}${order.currency ? ` ${order.currency}` : ' · currency unavailable'}`;
  return 'Unavailable';
}
...
function PortfolioTables({ snapshot }: { snapshot: PortfolioSnapshot }) {
  return <>
    <section className="card portfolio-section" aria-labelledby="portfolio-accounts-title">
      ...
      {snapshot.accounts.length ? <div className="table-scroll" tabIndex={0} aria-label="Portfolio accounts"><table><thead><tr><th>Account</th><th>Provider / environment</th><th>Health</th><th>Currency</th><th>Equity</th><th>Cash</th><th>Positions</th><th>Open orders</th></tr></thead><tbody>{snapshot.accounts.map(account => <tr key={account.connectionId}><td><strong>{account.label}</strong><small className="identity">{account.connectionId}</small></td><td>{account.providerId} · {account.environment}</td><td>{account.health.connection} · {account.health.reconciliation}</td><td>{account.accountCurrency ?? 'Unavailable'}</td><td>{valueText(account.equity)}</td><td>{valueText(account.cash)}</td><td>{account.positionsCount}</td><td>{account.openOrdersCount}</td></tr>)}</tbody></table></div> : <p className="muted">No account observations are available.</p>}
    </section>
    <section className="card portfolio-section" aria-labelledby="portfolio-holdings-title">
      ...
      {snapshot.holdings.length ? <div className="table-scroll" tabIndex={0} aria-label="Portfolio holdings"><table><thead><tr><th>Asset</th><th>Account</th><th>Quantity</th><th>Value</th><th>Unrealized P&amp;L</th><th>Observed / health</th></tr></thead><tbody>{snapshot.holdings.map((holding, index) => <tr key={`${holding.connectionId}:${holding.asset}:${index}`}><td><strong>{holding.instrumentId ?? holding.asset}</strong>{!holding.instrumentId && <small className="identity">No canonical instrument</small>}</td><td>{holding.accountLabel}</td><td>{holding.quantity ?? 'Unavailable'}</td><td>{valueText(holding.value)}</td><td>{holding.unrealizedPnl ? valueText(holding.unrealizedPnl) : 'Unavailable'}</td><td>{observationText(holding.observedAt, holding.health)}</td></tr>)}</tbody></table></div> : <p className="muted">No positions or balances are available.</p>}
    </section>
    <section className="card portfolio-section" aria-labelledby="portfolio-orders-title">
      ...
      {snapshot.openOrders.length ? <div className="table-scroll" tabIndex={0} aria-label="Portfolio open orders"><table><thead><tr><th>Asset</th><th>Account</th><th>Side</th><th>Quantity / notional</th><th>Broker state</th><th>Observed / health</th></tr></thead><tbody>{snapshot.openOrders.map(order => <tr key={order.brokerOrderId}><td>{order.instrumentId ?? order.asset}{!order.instrumentId && <small className="identity">No canonical instrument</small>}<small className="identity">{order.brokerOrderId}</small></td><td>{order.accountLabel}</td><td>{order.side}</td><td>{orderAmount(order)}</td><td>{order.status}</td><td>{observationText(order.observedAt, order.health)}</td></tr>)}</tbody></table></div> : <p className="muted">No open orders are available.</p>}
      <h3>Fills</h3>{snapshot.fills?.length ? <div className="table-scroll" tabIndex={0} aria-label="Portfolio fills"><table><thead><tr><th>Asset</th><th>Account</th><th>Quantity</th><th>Value</th><th>Observed / health</th></tr></thead><tbody>{snapshot.fills.map(fill => <tr key={fill.fillId}><td>{fill.instrumentId ?? fill.asset}{!fill.instrumentId && <small className="identity">No canonical instrument</small>}</td><td>{fill.accountLabel}</td><td>{fill.quantity}</td><td>{valueText(fill.value)}</td><td>{observationText(fill.observedAt, fill.health)}</td></tr>)}</tbody></table></div> : <p className="muted">Fills are unavailable from the connected providers.</p>}
    </section>
  </>;
}
