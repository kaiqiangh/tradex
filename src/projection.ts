...
import type { AccountConnection, AlpacaPaperOrderAttempt, AlpacaPaperOrderBook, DomainEvent, DomainProjection, GatewayState, ModelState, RiskPolicyState, Snapshot, Thread, Workspace } from '../shared/ipc-types.ts';
...
function projectionKind(p: DomainProjection) { return 'threadId' in p ? 'thread' : 'attemptId' in p ? 'alpaca-paper-order-attempt' : 'orders' in p ? 'alpaca-paper-order-book' : 'connectionId' in p ? 'account' : 'pinnedVersion' in p ? 'model-gateway' : 'chatgpt' in p ? 'model' : 'hardRules' in p ? 'risk' : 'workspace'; }

function snapshotOf<T>(value: unknown, kind: 'workspace' | 'account' | 'model-gateway' | 'model' | 'risk' | 'thread' | 'alpaca-paper-order-attempt' | 'alpaca-paper-order-book'): Projection<T> {
