...
import type { AccountConnection, AlpacaPaperOrderAttempt, AlpacaPaperOrderBook, DomainEvent, DomainProjection, GatewayState, ModelState, RiskPolicyState, Snapshot, Thread, Trading212DemoOrderAttempt, Workspace } from '../shared/ipc-types.ts';
...
function projectionKind(p: DomainProjection) { return 'threadId' in p ? 'thread' : 'attemptId' in p ? ('clientOrderId' in p ? 'alpaca-paper-order-attempt' : 'trading212-demo-order-attempt') : 'orders' in p ? 'alpaca-paper-order-book' : 'connectionId' in p ? 'account' : 'pinnedVersion' in p ? 'model-gateway' : 'chatgpt' in p ? 'model' : 'hardRules' in p ? 'risk' : 'workspace'; }

function snapshotOf<T>(value: unknown, kind: 'workspace' | 'account' | 'model-gateway' | 'model' | 'risk' | 'thread' | 'alpaca-paper-order-attempt' | 'trading212-demo-order-attempt' | 'alpaca-paper-order-book'): Projection<T> {
...
export function applyEvent<T extends DomainProjection>(current: Projection<T>, value: unknown): Projection<T> {
  ...
  const eventTypeValid = thread
    ...
    : book
    ? event.eventType === 'alpaca.paper.order.book.changed'
    : account
  ...
  if (event.aggregateId !== current.snapshot.aggregateId || event.aggregateType !== current.snapshot.aggregateType
      ...
      || ('connectionId' in old && 'orders' in old && (!book || p.connectionId !== old.connectionId || p.remoteAccountId !== old.remoteAccountId))
